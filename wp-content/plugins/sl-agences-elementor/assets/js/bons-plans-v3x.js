...
(function () {
    ...
    document.querySelectorAll('.slbp-wrapper').forEach(function (wrap) {
        ...
        if (agenceMs) {
            ...
            msPanel.style.zIndex   = '1000003'; // au-dessus du drawer (1000002), de l'overlay (1000001) et de la navbar mobile (1000000)
            ...
            function positionPanel() {
                ...
                var usableBottom = window.innerHeight - bottomObstruction;
                var top;
                var spaceBelow = usableBottom - rect.bottom;
                if (spaceBelow < panelH + 8 && rect.top > panelH + 8) {
                    top = rect.top - panelH - 4;
                } else {
                    top = rect.bottom + 4;
                }
                // Clamp dans la zone visible : pendant la transition du drawer le toggle peut
                // être hors écran (rect.left négatif), et sur petits écrans il peut se trouver
                // sous la navbar du bas → sans clamp le panel sort du viewport.
                top = Math.max(8, Math.min(top, usableBottom - panelH - 8));
                var width = Math.min(rect.width > 120 ? rect.width : 240, window.innerWidth - 16);
                var left  = Math.max(8, Math.min(rect.left, window.innerWidth - width - 8));
                msPanel.style.top   = top + 'px';
                msPanel.style.left  = left + 'px';
                msPanel.style.width = width + 'px';
            }

            function openMs() {
                // Afficher AVANT de positionner : offsetHeight réel (sinon fallback 200px faux).
                msPanel.style.display = 'block';
                positionPanel();
                // Le drawer glisse en 300ms : au clic le toggle peut être encore hors écran.
                // On repositionne après la transition pour rattraper le bon emplacement.
        ...
        var sbMark = null, ovMark = null;
        function openSidebar() {
            // Sortir drawer + overlay du stacking context du thème (.main-content a
            // position:relative + z-index:1) : tant qu'ils sont dedans, ils restent SOUS
            // la navbar mobile (.klb-mobile-bottom z=1000000) quel que soit leur z-index
            // → bloc Agences masqué et navbar cliquable par-dessus l'overlay.
            // Même technique que le panel du multiselect : déplacement vers <body>,
            // avec marqueurs pour restaurer la place d'origine à la fermeture.
            if (sidebar && sidebar.parentElement !== document.body) {
                sbMark = document.createComment('slbp-sb');
                sidebar.before(sbMark);
                document.body.appendChild(sidebar);
                if (sidebarOverlay) {
                    ovMark = document.createComment('slbp-ov');
                    sidebarOverlay.before(ovMark);
        ...
        function closeSidebar() {
            ...
            // Remettre drawer + overlay à leur place d'origine (layout desktop intact
            // si l'écran repasse au-dessus de 900px).
            if (sbMark && sidebar) { sbMark.replaceWith(sidebar); sbMark = null; }
            if (ovMark && sidebarOverlay) { ovMark.replaceWith(sidebarOverlay); ovMark = null; }
        }
        ...
        // ── PARTAGE D'UN BON PLAN (délégation : les cartes sont clonées dans la grille,
        // un listener direct serait perdu au clonage)
        wrap.addEventListener('click', function (e) {
            var btn = e.target.closest('.slbp-share-btn');
            if (!btn) return;
            e.preventDefault();
            e.stopPropagation();
            var card  = btn.closest('.slbp-card');
            var url   = (card && card.href) ? card.href : window.location.href;
            var titre = btn.dataset.titre || '';
            var prix  = btn.dataset.prix || '';
            var text  = 'Bon plan Santa Lucia : ' + titre + (prix ? ' à ' + prix : '') + ' 🔥';
            if (navigator.share) {
                navigator.share({ title: titre, text: text, url: url }).catch(function () {});
            } else {
        ...
        // ── TÉLÉCHARGEMENT PDF PAR AGENCE
        // Le href est recalculé au clic selon les agences cochées (toutes si aucune).
        ...
        if (pdfBtn) {
            pdfBtn.addEventListener('click', function () {
                ...
                if (filterAgences.length > 0) {
                    url += '&agence=' + encodeURIComponent(filterAgences.join(','));
                }
                pdfBtn.setAttribute('href', url);
                // la navigation par défaut suit le href mis à jour
            });
        }
