 ...
 */
/* Fix scroll horizontal mobile : off-canvas drawers dépassent le viewport */
...
(function () {
    ...
    document.querySelectorAll('.slbp-wrapper').forEach(function (wrap) {
        ...
        var agenceMs       = wrap.querySelector('.slbp-ms[data-filter="agence"]');
        ...
        // Agence — multi-select dropdown
        if (agenceMs) {
            var msToggle   = agenceMs.querySelector('.slbp-ms-toggle');
            var msDropdown = agenceMs.querySelector('.slbp-ms-dropdown');
            var msLabel    = agenceMs.querySelector('.slbp-ms-label');
            var msOptions  = agenceMs.querySelectorAll('.slbp-ms-option input[type="checkbox"]');

            // ── Déplacer le dropdown vers <body> pour échapper à overflow:hidden
            document.body.appendChild(msDropdown);
            msDropdown.style.display  = 'none';
            msDropdown.style.position = 'fixed';
            msDropdown.style.zIndex   = '999999';

            function positionMsDropdown() {
                var rect = msToggle.getBoundingClientRect();
                msDropdown.style.top   = (rect.bottom + 4) + 'px';
                msDropdown.style.left  = rect.left + 'px';
                msDropdown.style.width = rect.width + 'px';
            }
            ...
            document.addEventListener('click', function (e) {
                if (!agenceMs.contains(e.target) && !msDropdown.contains(e.target)) {
                    closeMs();
                }
            });
            window.addEventListener('scroll', function () {
                if (agenceMs.classList.contains('is-open')) positionMsDropdown();
            }, true);
            ...
            function syncAgenceMs() {
                ...
                var selected = [];
                msOptions.forEach(function (cb) {
                    if (cb.checked) {
                        filterAgences.push(cb.value);
                        selected.push(cb.closest('.slbp-ms-option').querySelector('span').textContent.trim());
                    }
                });
            ...
            msOptions.forEach(function (cb) {
                cb.addEventListener('change', syncAgenceMs);
            });
        }
