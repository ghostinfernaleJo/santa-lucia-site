...
function sl_cwoo_add_whatsapp_button() {
    ...
    // esc_attr, pas esc_url : esc_url() supprime %0a/%0A (retours a la ligne encodes)
    // meme dans un texte deja urlencode() -> le message WhatsApp arrivait sans sauts
    // de ligne. $whatsapp_url est notre propre construction (hote wa.me fixe).
    echo '<a href="' . esc_attr($whatsapp_url) . '" target="_blank" rel="noopener" style="display: flex; align-items: center; justify-content: center; gap: 8px; background: #25D366; color: #fff; text-decoration: none; padding: 12px; border-radius: 6px; font-weight: 700; font-size: 15px; margin-top: 15px; margin-bottom: 15px; transition: opacity 0.2s; max-width: 300px;">';
