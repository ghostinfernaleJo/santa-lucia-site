    ...
    <div class="slbp-single-layout" style="display: grid; grid-template-columns: 1fr 1fr; gap: 40px; background: #fff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.06); border: 1px solid #eee;">
        ...
        <div class="slbp-single-content" style="padding: 40px 40px 40px 0; display: flex; flex-direction: column; justify-content: center;">
            ...
            <div style="margin-top: auto; padding-top: 30px; border-top: 1px solid #eee;">
                <p style="margin: 0 0 15px; font-weight: 600; font-size: 14px; color: #555;">Commander ce bon plan :</p>
                <div style="display: flex; gap: 10px;">

                    <!-- WhatsApp -->
                    <a href="<?php echo esc_attr( $whatsapp_url ); // esc_attr, pas esc_url : esc_url() supprime %0a/%0A (retours a la ligne encodes) meme dans un texte deja urlencode() -> le message WhatsApp arrivait sans sauts de ligne. $whatsapp_url est notre propre construction (hote wa.me fixe), esc_attr suffit. ?>" target="_blank" rel="noopener" style="flex: 1; display: flex; align-items: center; justify-content: center; gap: 8px; background: #25D366; color: #fff; text-decoration: none; padding: 12px; border-radius: 6px; font-weight: 700; font-size: 15px; transition: opacity 0.2s;">
                    ...
                    <?php
                    $cart_btn = function_exists( 'sl_bp_cart_button_html' ) ? sl_bp_cart_button_html( $p_id ) : '';
                    if ( $cart_btn ) :
                    ?>
                    <div style="flex: 1;">
                        <?php echo $cart_btn; // phpcs:ignore -- markup interne de confiance ?>
                    </div>
                    <?php endif; ?>
...
<style>
.slbp-single-wrapper .slbp-cart-wrap{margin-top:0;height:100%;}
.slbp-single-wrapper .slbp-add-cart{height:100%;padding:12px;font-size:15px;border-radius:6px;}
