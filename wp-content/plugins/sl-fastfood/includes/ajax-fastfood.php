...
function sl_ff_ajax_save_planning() {
    ...
    $agences_post = array_map( 'sanitize_title', (array) get_post_meta( $post_id, '_sl_ff_agence' ) );
    if ( ! current_user_can( 'manage_options' ) && ! current_user_can( 'sl_ff_all_agencies' ) ) {
        ...
        if ( ! $agence_user || $agence !== sanitize_title( $agence_user ) || ! in_array( $agence, $agences_post, true ) ) {
            wp_send_json_error( 'Acces refuse' );
        }
    } elseif ( $agence && ! in_array( $agence, $agences_post, true ) ) {
        wp_send_json_error( 'Agence invalide pour ce repas' );
    }
