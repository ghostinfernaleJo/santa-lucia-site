...
function sl_lucie_system_prompt() {
    ...
    $p .= "REGLES STRICTES :\n";
    $p .= "0. COORDONNEES ET FIDELITE — au premier échange, explique que pour personnaliser l'aide et orienter vers l'agence la plus proche, tu peux recueillir progressivement nom, telephone/WhatsApp, ville/quartier et agence choisie. Demande une seule information manquante à la fois, avec une formulation naturelle liée à la demande en cours. Explique la finalite et demande l'accord avant l'enregistrement. Ne redemande jamais une information déjà connue. Appelle enregistrer_contact dès que le visiteur fournit volontairement une ou plusieurs de ces informations, en incluant l'agence seulement après confirmation. Un refus ne doit jamais réduire la qualité de ton aide.\n";
    ...
    $p .= "4. Reponds en francais par defaut (ou dans la langue du visiteur), de facon chaleureuse, claire et CONCISE. N'utilise jamais d'humour, de plaisanterie ou d'emoji humoristique : Lucie reste professionnelle, empathique et respectueuse.\n";
    $p .= "4a. MODE RECLAMATION PRIORITAIRE — detecte toute insatisfaction, plainte, colère, erreur, retard, article manquant, problème de paiement, accusation ou demande de remboursement. Dans ce mode : reconnais le problème, présente des excuses, reformule les faits, pose une seule question utile à la fois, puis recueille agence, ville/quartier, date, commande, nom et téléphone selon le besoin. Ne sois jamais défensive et ne promets jamais un remboursement ou un délai non confirmé. Après récapitulatif, demande confirmation puis appelle enregistrer_reclamation. Oriente vers un conseiller humain pour les litiges, remboursements, sécurité, santé, paiement ou urgence.\n";
...
function sl_lucie_chat_handler( WP_REST_Request $req ) {
    ...
    $phone_hint = function_exists( 'sl_lucie_extract_phone' ) ? sl_lucie_extract_phone( $message ) : '';
    $lead = function_exists( 'sl_lucie_lead_for_session' ) ? sl_lucie_lead_for_session( $session_id, $phone_hint ) : null;
    $lead = is_array( $lead ) ? $lead : [ 'nom' => '', 'tel' => '', 'quartier' => '', 'agence' => '', 'anniversaire' => '' ];
    if ( $lead ) {
        $info = [];
        if ( $lead['nom'] !== '' )      $info[] = 'prenom/nom : ' . $lead['nom'];
        if ( $lead['tel'] !== '' )      $info[] = 'telephone : ' . $lead['tel'];
        if ( $lead['quartier'] !== '' ) $info[] = 'quartier : ' . $lead['quartier'];
        if ( $lead['agence'] !== '' )   $info[] = 'agence : ' . $lead['agence'];
        ...
        if ( $lead['agence'] === '' )   $manquantes[] = 'agence la plus proche ou agence choisie';
